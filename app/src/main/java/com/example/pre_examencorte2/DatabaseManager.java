...
public class DatabaseManager extends SQLiteOpenHelper {
    ...
    public void agregarUsuario(String nombreUsuario, String correo, String contraseña, byte[] foto) {
    ...
    public Usuario obtenerUsuarioPorId(int usuarioId) {
        ...
        Cursor cursor = db.query(
                TABLE_USUARIO,
                new String[]{COLUMN_ID, COLUMN_NOMBRE_USUARIO, COLUMN_CORREO, COLUMN_CONTRASEÑA, COLUMN_FOTO},
                COLUMN_ID + "=?",
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            if (columnIndexId != -1 && columnIndexNombreUsuario != -1 && columnIndexCorreo != -1
                    && columnIndexContraseña != -1 && columnIndexFoto != -1) {
                usuario = new Usuario(
    ...
    public List<Usuario> buscarUsuarios(String query) {
        ...
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + TABLE_USUARIO +
                " WHERE " + COLUMN_NOMBRE_USUARIO + " LIKE '%" + query + "%' OR " +
                COLUMN_CORREO + " LIKE '%" + query + "%'", null);
